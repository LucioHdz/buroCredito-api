...
Method.create = (newMethod, callback) => {
    database.query(`INSERT INTO tipopago VALUES(
        '${newMethod.id}',
...
Method.update = (id, data, callback) => {
    database.query(`UPDATE tipopago SET
    nombrePago = '${data.name}',
    status = ${data.status}
    WHERE idTipoPago = '${id}'`, (err, res) => {
        if (err) callback(err, null);
...
Method.delete = (id, callback) => {
    database.query(`DELETE FROM tipopago WHERE idTipoPago = '${id}'`, (err, res) => {
        if (err) callback(err, null);
